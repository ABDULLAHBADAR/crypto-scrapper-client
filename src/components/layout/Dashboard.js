...
const Dashboard = ({ auth, logoutUser }) => {
  ...
  const handleSubscription = async () => {
    ...
    const clients = await fetch(
      `https://crypto-scrapper-server-xmet.onrender.com/api/users/getUser?email=${user.email}`
    );
  ...
  useEffect(() => {
    const ws = new WebSocket(`wss://crypto-scrapper-server-xmet.onrender.com/${clientId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const clients = await fetch('https://crypto-scrapper-server-xmet.onrender.com/sockets', {
        method: 'POST',
        headers: {
      ...
      const response = await fetch("https://crypto-scrapper-server-xmet.onrender.com/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
